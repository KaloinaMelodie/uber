...
(function () {
  ...
  function move(dx, dy, ax, ay) {
    ctx.fillText("X", dx * 50, dy * 50);
  ...
  function init() {
    ...
    document.getElementById('go').onclick = function () {
      for (var e = 0; e < points.length - 1; i++) {
        var depart = points[0];
        var arrive = points[1];

        if (dx >= points[0][0] && dy >= points[0][1]) {
          var timer = setInterval(move(depart[0], depart[1], arrive[0], arrive[1]), 1000);
        }
      }
    };
  }
...
/******/ });
//# sourceMappingURL=gridSolution-c76a115e5704cc6b582b.js.map